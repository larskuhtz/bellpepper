...
//!
//! Proving takes about 4 times longer than for SHA256.
...
fn get_sha512_iv() -> Vec<UInt64> {
    IV512.iter().map(|&v| UInt64::constant(v)).collect()
}
...
mod test {
    ...
    fn test_full_block() {
        ...
        assert!(cs.is_satisfied());
        assert_eq!(cs.num_constraints() - 512, 66611);
    }
    ...
    fn test_full_hash() {
        ...
        assert!(cs.is_satisfied());
        assert_eq!(cs.num_constraints() - 512, 113617);
    }
